...
const pinFileStreamToIpfs = async (pinata, file: StreamPinata, name?: string): Promise<string> => {
    const options = { ...defaultOptions, pinataMetadata: { name } };
    const result = await pinata.pinFileToIPFS(file, options);
    return result.IpfsHash;
};

export const uploadAndPinIpfsMetadata = async (pinata, metadataFields): Promise<string> => {
    const options = {
        ...defaultOptions,
...
export const pinSingleMetadataFromDir = async (
    ...
    try {
        ...
        const metadata = { ...metadataBase, name, mediaUri: `ipfs://ipfs/${imageCid}` };
...
export const pinSingleWithThumbMetadataFromDir = async (
    ...
    try {
        ...
        const metadata = { ...metadataBase, name, mediaUri: `ipfs://ipfs/${mainCid}`, thumbnailUri: `ipfs://ipfs/${thumbCid || mainCid}` };
        const metadataCid = await uploadAndPinIpfsMetadata(pinata, metadata);
        await sleep(500);
        logger.info(`NFT ${name} METADATA: `, metadataCid);
