...
import type { DeriveApi, DeriveBalancesAccount, DeriveReferendum, DeriveReferendumVote, DeriveReferendumVotes } from '@polkadot/api-derive/types';
...
import { calcVotes, getStatus } from './utils.js';
...
function votesCurr (api: DeriveApi, referendumId: BN): Observable<DeriveReferendumVote[]> {
  return this.api.query.democracy.votingOf.entries().pipe(
    map((allVoting: any): DeriveReferendumVote[] => {
...
export function referendumsInfo (instanceId: string, api: DeriveApi): (ids: BN[]) => Observable<DeriveReferendum[]> {
  return memo(instanceId, (ids: BN[]): Observable<DeriveReferendum[]> =>
    ids.length
      ? this.api.query.democracy.referendumInfoOf.multi(ids).pipe(
